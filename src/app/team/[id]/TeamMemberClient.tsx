      ...
      <main className="container mx-auto px-6 pt-40">
        <MotionDiv initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
          ...
          <div className="bg-white/60 dark:bg-black/40 backdrop-blur-3xl border border-white dark:border-white/10 rounded-3xl p-8 md:p-14 mb-16 shadow-2xl shadow-primary-900/5 dark:shadow-black/50 relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-[500px] h-[500px] opacity-20 dark:opacity-40 rounded-full mix-blend-screen pointer-events-none transition-transform duration-1000 group-hover:scale-110" style={{ background: `radial-gradient(circle, ${member.gradient?.split(',')[1] || 'transparent'} 0%, transparent 70%)` }}></div>
              ...
              <div className="relative w-40 h-40 md:w-56 md:h-56 shrink-0">
                ...
                {member.avatar ? (
                  ...
                  <div 
                    ...
                    style={{ background: member.gradient || '#333' }}
                  >
                    {member.initials}
                  </div>
                )}
              ...
              <div className="flex-1 w-full">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6 border-b border-gray-200 dark:border-white/10 pb-6">
                  <div>
                    ...
                    <p className="text-xl md:text-2xl text-transparent bg-clip-text font-bold" style={{ backgroundImage: member.gradient || 'none' }}>{member.role}</p>
                  </div>
                  
                  <div className="flex gap-3">
                    {member.github && (
                      <a href={member.github} target="_blank" rel="noopener noreferrer" className="p-3 bg-white dark:bg-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-700 text-gray-700 dark:text-gray-300 rounded-xl border border-gray-200 dark:border-zinc-700 shadow-sm transition-all hover:-translate-y-1" aria-label="GitHub">
                    ...
                    {member.linkedin && (
                      <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="p-3 bg-white dark:bg-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-700 text-[#0077b5] rounded-xl border border-gray-200 dark:border-zinc-700 shadow-sm transition-all hover:-translate-y-1" aria-label="LinkedIn">
                ...
                <div className="flex flex-wrap gap-4 text-sm font-medium mb-8">
                  ...
                  {member.yearsExperience && (
                    <span className="flex items-center gap-2 bg-white/50 dark:bg-black/40 px-4 py-2 rounded-lg border border-gray-200 dark:border-white/5 text-gray-700 dark:text-gray-300">
                      <Briefcase size={16} className="text-primary-500" /> {member.yearsExperience} {language === 'ar' ? 'خبرة' : 'Experience'}
                    </span>
                  )}
                </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
            ...
            <MotionDiv initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
              <div className="bg-white/60 dark:bg-black/40 backdrop-blur-2xl rounded-3xl p-8 border border-white dark:border-white/10 shadow-xl">
                <h2 className="text-2xl font-bold mb-8 flex items-center gap-3 text-gray-900 dark:text-white">
                  <Award className="text-primary-500" /> {language === 'ar' ? 'الترسانة التقنية' : 'Technical Arsenal'}
                </h2>
          ...
          <div className="xl:col-span-8 space-y-10">
            ...
            <MotionDiv initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.3 }}>
              <div className="bg-white/60 dark:bg-black/40 backdrop-blur-2xl rounded-3xl p-8 border border-white dark:border-white/10 shadow-xl">
                <h2 className="text-2xl font-bold mb-8 flex items-center gap-3 text-gray-900 dark:text-white">
                  <Calendar className="text-primary-500" /> {language === 'ar' ? 'الرحلة المهنية' : 'Professional Journey'}
                </h2>
                
                <div className={`relative ${language==='ar' ? 'pr-4 border-r-2' : 'pl-4 border-l-2'} border-gray-200 dark:border-zinc-700 space-y-10`}>
                  {member.experience?.map((exp, idx) => (
                    <div key={idx} className="relative group">
                      <div className={`absolute ${language==='ar' ? '-right-[23px]' : '-left-[23px]'} top-1 h-3 w-3 rounded-full border-2 ${exp.current ? 'bg-primary-500 border-primary-100 dark:border-primary-900 shadow-[0_0_12px_rgba(59,130,246,0.6)]' : 'bg-gray-300 dark:bg-zinc-600 border-white dark:border-zinc-800'} transition-all group-hover:scale-150`}></div>
                      <div>
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-2">
                          <h3 className="text-xl font-bold text-gray-900 dark:text-white">{exp.title}</h3>
                          {exp.current && (
                            <span className="text-xs font-bold px-2 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 rounded-md">
                              {language === 'ar' ? 'الحالي' : 'Present'}
                            </span>
                          )}
                        </div>
            ...
            {member.projects && member.projects.length > 0 && (
              <MotionDiv initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }}>
                <div className="bg-white/60 dark:bg-black/40 backdrop-blur-2xl rounded-3xl p-8 border border-white dark:border-white/10 shadow-xl">
                  <h2 className="text-2xl font-bold mb-8 flex items-center gap-3 text-gray-900 dark:text-white">
                    <FolderGit2 className="text-primary-500" /> {language === 'ar' ? 'أبرز المشاريع' : 'Featured Projects'}
                  </h2>
